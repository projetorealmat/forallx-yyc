...
\begin{itemize}
  ...
  \item \textquotedblleft download\textquotedblright{} (baixar) para os links das versões em PDF; e
...
As expressões `\blank{}` e `\ifeff` são usadas em todo o livro. Na versão HTML, `\blank{}` inclui a palavra invisível `lacuna`, que os leitores de tela devem ler em voz alta. A macro `\ifeff` é expandida como «se, e somente se,». Como essa expressão já está em português, a versão HTML não precisa de uma codificação fonética especial.
...
\end{center}
A descrição da imagem deve ser: \textquotedblleft Há três formas: a primeira é um círculo cinza identificado como A, há algum espaço vazio, a segunda é um círculo branco identificado como C e a quarta é um quadrado branco identificado como D.\textquotedblright{}
