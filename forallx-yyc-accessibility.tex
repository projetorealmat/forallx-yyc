...
As expressões `\blank{}` e `\ifeff` (`if` com dois `f`) são usadas em todo o livro. Na versão HTML, `\blank{}` inclui a palavra soletrada invisível `blank`, que os leitores de tela devem ler em voz alta. A abreviação `\ifeff` significa \textquotedblleft{}se, e somente se\textquotedblright{}. Na versão HTML, ela é codificada usando um caractere invisível de junção de palavras sem espaço, que deve fazer os leitores de tela pronunciarem `\ifeff` como `if-eff` ou `aye-eff-eff`.
