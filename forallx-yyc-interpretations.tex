...
\item[\atom{C}{x}] \gap{x} ensina Lógica III em Calgary
	\end{ekey}
Essa estipulação não transporta o \emph{significado} do predicado em português para o nosso predicado da LPO. Estamos simplesmente estipulando algo como isto:
	\begin{itemize}
...
\item para cada predicado que decidamos considerar (exceto \textquotedblleft $=$\textquotedblright{}), uma especificação de quais coisas (e em que ordem) tornam o predicado verdadeiro (sua \define{extens\~ao}).
...
Para que \textquotedblleft $\exists x\,\atom{A}{x,x} \eif \atom{B}{d}$\textquotedblright{} seja falsa, o antecedente (\textquotedblleft $\exists x\, \atom{A}{x,x}$\textquotedblright{}) deve ser verdadeiro e o consequente (\textquotedblleft $\atom{B}{d}$\textquotedblright{}) deve ser falso. Para construir uma interpretação assim, começamos especificando um domínio. Manter o domínio pequeno facilita especificar a quais objetos os predicados se aplicam; por isso, começaremos com um domínio que tenha apenas um membro. Concretamente, digamos que esse membro seja apenas a cidade de Paris.
	\begin{ekey}
...
\item $R$ é antissimétrica: \[\lnot\exists x\exists y((\atom{R}{x,y} \eand \atom{R}{y,x}) \eand \enot x=y)\] ou, de modo equivalente, \[\forall x\forall y((\atom{R}{x,y}
