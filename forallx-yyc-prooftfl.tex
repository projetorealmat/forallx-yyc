...
\end{fitchproof}
com a citação invertida, para refletir a ordem dos conjuntivos. De modo mais geral, eis nossa regra de introdução da conjunção:
\factoidbox{
\begin{fitchproof}
	\have[m]{a}{\metav{A}}
	\have[n]{b}{\metav{B}}
...
apenas que ambos os conjuntivos estejam disponíveis em algum ponto anterior da
...
\end{fitchproof}
Da premissa, podemos obter cada um dos conjuntivos por meio de {\eand}E. A prova agora é:
\begin{fitchproof}
	\hypo{ab}{{[}(A\eor B)\eif(C\eor D){]} \eand {[}(E \eor F) \eif (G\eor H){]}}\PR
...
\end{fitchproof}
Da premissa, podemos obter cada um dos conjuntivos aplicando~$\eand$E duas vezes. Podemos então aplicar $\eand$E mais duas vezes; assim, nossa prova fica:
\begin{fitchproof}
	\hypo{ab}{A \eand (B \eand C)}\PR
	\have{a}{A} \ae{ab}
