...
Daqui em diante, abreviaremos \textquotedblleft{}forma normal disjuntiva\textquotedblright{} por \textquotedblleft{}FND\textquotedblright{}.
...
\end{tabular}
\end{center}
Provavelmente esse novo conectivo não corresponderia a nenhuma expressão natural em português (pelo menos não da maneira como $\eand$ corresponde a \textquotedblleft{}e\textquotedblright{}). Mas surge uma pergunta: se quiséssemos empregar um conectivo com essa tabela-verdade característica, teríamos de acrescentar um conectivo \emph{novo} à LVF? Ou podemos nos virar com os conectivos que \emph{já temos} (como podemos fazer com o conectivo \textquotedblleft{}nem\dots nem\textquotedblright{}, por exemplo)?
...
\end{tabular}
\end{center}
Às vezes ele é chamado de \textquotedblleft{}seta de Peirce\textquotedblright{} (o próprio Peirce o chamou de \textquotedblleft{}ampheck\textquotedblright{}). Mais frequentemente, porém, é chamado de \textquotedblleft{}nor\textquotedblright{}, pois sua tabela-verdade característica é a negação de $\eor$, isto é, de \textquotedblleft{}nem\dots{} nem\dots\textquotedblright{}.
	\factoidbox{
...
É claro que isso é mais difícil de demonstrar do que no caso dos conectivos primitivos. Por exemplo, o conectivo \textquotedblleft{}ou exclusivo\textquotedblright{} não tem um T na primeira linha de sua tabela-verdade característica, e portanto o método usado acima já não basta para mostrar que ele não pode expressar todas as tabelas-verdade. Também é mais difícil mostrar que, por exemplo, $\eiff$ e $\enot$ \emph{juntos} não são funcionalmente completos.
...
Para começar, precisamos tornar mais precisa a ideia de \textquotedblleft{}levar ao erro\textquotedblright{}. Diremos que uma linha de uma demonstração é \define{brilhante} \emph{\ifeff} as suposições das quais essa linha depende implicam a sentença escrita na linha.\footnote{A palavra \textquotedblleft{}brilhante\textquotedblright{} não é padrão entre os lógicos.} Para ilustrar a ideia, considere:
	\begin{fitchproof}
...
$\Delta_i$ (\textquotedblleft{}delta\textquotedblright{}) abreviará as suposições (se houver) das quais a linha~$i$ depende em nossa demonstração na LVF (o contexto indicará qual demonstração na LVF temos em mente). Isso inclui todas as premissas de nossa demonstração e todas as suposições de subdemonstrações que ainda estão abertas na linha~$i$.
