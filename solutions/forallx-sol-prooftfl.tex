...
\end{fitchproof}
\vfill
\noindent\myanswer{$\eand$E on line 4 can't be applied to line~1, since it is not of the form $\metav{A} \eand \metav{B}$.  `$A$' could be obtained by $\eand$E, but from line~2.}

\myanswer{$\enot$E on line~6 illicitly refers to a line from a closed subproof (line~3).}
\hfill
\columnbreak
\begin{fitchproof}[arrayenv=tabular]
...
\noindent\myanswer{Corresponding argument: \\$A \eif D \therefore (A \eand B) \eif (D \eor E)$}
...
\noindent\myanswer{Corresponding argument:\\
...
\problempart
Show that each of the following pairs of sentences are interderivable: %TB: a bit of a hack here, to try to get the multicols thing working nicer.
...
\\\myanswer{If $\metav{A} \proves \metav{B}$, then $(\metav{A} \eand \metav{C})\proves \metav{B}$. After all, if $\metav{A} \proves \metav{B}$, then there is some proof with assumption $\metav{A}$ that ends with $\metav{B}$, and no undischarged assumptions other than $\metav{A}$. Now, if we start a proof with assumption $(\metav{A} \eand \metav{C})$, we can obtain $\metav{A}$ by $\eand$E. We can now copy and paste the original proof of $\metav{B}$ from $\metav{A}$, adding 1 to every line number and line number citation. The result will be a proof of $\metav{B}$ from assumption $\metav{A} \eand\metav{C}$.}

\myanswer{However, we cannot prove much from $(\metav{A} \eor \metav{C})$. After all, it might be impossible to prove $\metav{B}$ from $\metav{C}$.}
...
\\\myanswer{Consider the sentence $\metav{A} \eiff \metav{B}$. Suppose we can show that this is a theorem. So we can prove it, with no assumptions, in $m$ lines, say. Then if we assume $\metav{A}$ and copy and paste the proof of $\metav{A} \eiff \metav{B}$ (changing the line numbering), we will have a deduction of this shape:
\begin{fitchproof}
\hypo{a}{\metav{A}}
\have[m][1]{m}{\metav{A} \eiff \metav{B}}
\have{b}{\metav{B}}\be{m, a}
\end{fitchproof}
This will show that $\metav{A} \proves \metav{B}$. In exactly the same way, we can show that $\metav{B} \proves \metav{A}$. So if we can show that $\metav{A} \eiff \metav{B}$ is a theorem, we can show that $\metav{A}$ and $\metav{B}$ are interderivable.}

\myanswer{Conversely, suppose we can show that $\metav{A}$ and $\metav{B}$ are interderivable. Then we can prove $\metav{B}$ from the assumption of $\metav{A}$ in $m$ lines, say, and prove $\metav{A}$ from the assumption of $\metav{B}$ in $n$ lines, say. Copying and pasting these proofs together (changing the line numbering where appropriate), we obtain:
\begin{fitchproof}
\open
	\hypo{a}{\metav{A}}
	\have[m]{b1}{\metav{B}}
...
\end{fitchproof}
This shows that $\metav{A} \eiff \metav{B}$ is a theorem.}

\myanswer{There was nothing special about $\metav{A}$ and $\metav{B}$ in this. So what this shows is that the problem of showing that two sentences are interderivable is, essentially, the same problem as showing that a certain kind of sentence (a biconditional) is a theorem.}
