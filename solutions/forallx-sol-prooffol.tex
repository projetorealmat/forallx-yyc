...
\chapter{Regras básicas para a LPO}\setcounter{ProbPart}{0}
\problempart
Explique por que essas duas \textquotedblleft{}demonstrações\textquotedblright{} estão incorretas. Além disso, forneça interpretações que invalidem as formas argumentativas falaciosas que essas \textquotedblleft{}demonstrações\textquotedblright{} consagram:
\begin{multicols}{2}
	...
	\end{fitchproof}
\vfill
\noindent\myanswer{Ao usar $\forall$I, é preciso substituir \emph{todos} os nomes pela nova variável. Portanto, a linha 3 é ilegítima. Como contra-interpretação, considere:
\begin{center}
\begin{tikzpicture}
\node (atom4) at (0,0) {1};
\node (atom5) at (2,0) {2};
...
\end{tikzpicture}
\bmlDescription{O número 1 está conectado a si mesmo por uma seta. O
número 2 também está conectado a si mesmo por uma seta. 1 e 2 não estão
conectados de nenhuma outra maneira.}
\end{center}}
\columnbreak
	\begin{fitchproof}[arrayenv=tabular]
	...
	\end{fitchproof}
\vfill
\noindent\myanswer{A constante instanciadora, $a$, ocorre na linha (linha 2) à qual $\exists$E deve ser aplicada na linha 5. Portanto, o uso de $\exists$E na linha 5 é ilegítimo. Como contra-interpretação, considere:
\begin{center}
\begin{tikzpicture}
\node (atom4) at (0,0) {1};
\node (atom5) at (2,0) {2};
...
\end{tikzpicture}
\bmlDescription{Os números 1 e 2 estão conectados por setas em ambas as direções.}
\end{center}}
\end{multicols}
...
As três demonstrações a seguir estão sem suas citações (regra e números de linha). Acrescente-as para transformá-las em demonstrações legítimas.
\begin{compactlist}
...
Em \hyperref[pr.BarbaraEtc]{exercício \ref*{s:MoreMonadic}A},
consideramos quinze figuras silogísticas da lógica aristotélica.
Forneça demonstrações para cada uma das formas argumentativas. OBS.: será
\emph{muito} mais fácil se você simbolizar, por exemplo, \textquotedblleft{}Nenhum F é G\textquotedblright{} como
$\forall x (\atom{F}{x} \eif \enot \atom{G}{x})$. \\\myanswer{Demonstraremos os quatro silogismos da Figura I; os demais são
\emph{extremamente}
semelhantes.
\begin{compactlist}
\item \textbf{Barbara}
...
\end{fitchproof}
\item\textbf{Celerant} é exatamente como Barbara, substituindo $F$ por $\enot F$ em toda parte.
\item\textbf{Ferio}
\begin{fitchproof}
...
\end{fitchproof}
\item\textbf{Darii} é exatamente como Ferio, substituindo $\enot F$ por $F$ em toda parte.\end{compactlist}}

\

\problempart
\label{pr.BarbaraEtc.proof2}
Aristóteles e seus sucessores identificaram outras formas silogísticas que dependiam da \textquotedblleft{}importação existencial\textquotedblright{}. Simbolize cada uma das formas argumentativas a seguir na LPO e ofereça demonstrações.
\begin{compactlist}
\newpage	\item \textbf{Barbari.} Algo é H. Todo G é F. Todo H é G. Logo: algum H é F
...
\\	A demonstração é exatamente como a de Barbari, substituindo $F$ por $\enot F$ em toda parte.}
...
\newpage	\item \textbf{Camestros.} Algo é H. Todo F é G. Nenhum H é G. Logo: algum H não é F
...
\end{fitchproof}}
	...
	\\\myanswer{$\exists x\,\atom{G}{x}, \forall x (\atom{G}{x} \eif \atom{F}{x}), \forall x(\atom{G}{x} \eif \atom{H}{x}) \therefore \exists x (\atom{H}{x} \eand \atom{F}{x})$\\
		A demonstração é exatamente como a de Felapton, substituindo $\enot F$ por $F$ em toda parte.}

\newpage	\item \textbf{Calemos.} Algo é H. Todo F é G. Nenhum G é H. Logo: algum H não é F
...
\newpage	\item \textbf{Bamalip.} Algo é F. Todo F é G. Todo G é H. Logo: algum H é F
...
Para cada uma das afirmações a seguir, forneça uma demonstração na LPO que mostre que ela é verdadeira.
...
\problempart
\label{pr.likes}
Escreva uma chave de simbolização para o argumento a seguir, simbolize-o e demonstre-o:
\begin{earg}
\item Há alguém que gosta de todas as pessoas que gostam de todas as pessoas de quem ela
gosta.
\item[\texttherefore] Há alguém que gosta de si mesma.
\end{earg}
\myanswer{Chave de simbolização:
\begin{ekey}
\item[\text{domínio}] todas as pessoas
...
Para cada um dos pares de sentenças a seguir: se forem interderiváveis, forneça demonstrações que mostrem isso. Se não forem, construa uma interpretação que mostre que não são logicamente equivalentes.
\begin{compactlist}
\item $\forall x\,\atom{P}{x} \eif \atom{Q}{c}, \forall x (\atom{P}{x} \eif \atom{Q}{c})$ \hfill \myanswer{Não são logicamente equivalentes}
\\\myanswer{Contra-interpretação: tome como domínio os números $1$ e $2$. Faça $c$ designar $1$. Faça $\atom{P}{x}$ ser verdadeira somente para $1$. Faça $\atom{Q}{x}$ ser verdadeira somente para $2$.}
\item $\forall x\forall y \forall z \atom{B}{x,y,z}, \forall x\,\atom{B}{x,x,x}$\hfill \myanswer{Não são logicamente equivalentes}
\\\myanswer{Contra-interpretação: tome como domínio os números $1$ e $2$. Faça $\atom{B}{x,y,z}$ ser verdadeira somente para \ntuple{1,1,1} e \ntuple{2,2,2}.}
\item $\forall x\forall y\,\atom{D}{x,y}, \forall y\forall x\,\atom{D}{x,y}$ \hfill \myanswer{interderiváveis\begin{multicols}{2}
\begin{fitchproof}[arrayenv=tabular]
...
\end{fitchproof}
\end{multicols}}
\item $\exists x\forall y\,\atom{D}{x,y}, \forall y\exists x\,\atom{D}{x,y}$ \hfill \myanswer{Não são logicamente equivalentes}
\\\myanswer{Contra-interpretação: tome como domínio os números $1$ e $2$. Faça $Dxy$ valer somente para \ntuple{1,2} e \ntuple{2,1}. Isso é representado no diagrama:
\begin{center}
\begin{tikzpicture}
\node (atom4) at (0,0) {1};
\node (atom5) at (2,0) {2};
...
\bmlDescription{Os números 1 e 2 estão conectados por setas em ambas as direções.}
\end{center}}
\item $\forall x (\atom{R}{c,a} \eiff \atom{R}{x,a}), \atom{R}{c,a} \eiff \forall x\,\atom{R}{x,a}$ \hfill \myanswer{Não são logicamente equivalentes}
\\\myanswer{Contra-interpretação: considere o diagrama a seguir, fazendo $a$ designar 1 e $c$ designar 2:
\begin{center}
\begin{tikzpicture}
\node (atom4) at (0,0) {1};
\node (atom5) at (2,0) {2};
...
\end{tikzpicture}
\bmlDescription{O número 1 está conectado a si mesmo por uma seta. O
número 2 não está conectado por nenhuma seta.}
\end{center}}
\end{compactlist}
...
Para cada um dos argumentos a seguir: se for válido na LPO, forneça uma demonstração. Se for inválido, construa uma interpretação que mostre que é inválido.
\begin{compactlist}
\item $\exists y\forall x\,\atom{R}{x,y} \therefore \forall x\exists y\,\atom{R}{x,y}$ \hfill \myanswer{Válido
\begin{fitchproof}
...
\end{fitchproof}}
\item $\forall x\exists y\,\atom{R}{x,y} \therefore  \exists y\forall x\,\atom{R}{x,y}$ \hfill \myanswer{Inválido
\\Contra-interpretação: tome como domínio os números $1$ e $2$. Faça $Rxy$ ser verdadeira para $1,2$ e para $2,1$ (mas não para $1,1$ nem para $2,2$).}
\item $\exists x(\atom{P}{x} \eand \enot \atom{Q}{x}) \therefore \forall x(\atom{P}{x} \eif \enot \atom{Q}{x})$ \hfill \myanswer{Inválido
\\Contra-interpretação: tome como domínio os números $1$ e $2$. Faça $Px$ ser verdadeira para tudo no domínio. Faça $Qx$ ser verdadeira somente para $2$.}
\item $\forall x(\atom{S}{x} \eif \atom{T}{a}), \atom{S}{d} \therefore \atom{T}{a}$ \hfill \myanswer{Válido
\begin{fitchproof}
...
\end{fitchproof}}
\item $\forall x(Ax\eif \atom{B}{x}), \forall x(\atom{B}{x} \eif \atom{C}{x}) \therefore \forall x(\atom{A}{x} \eif \atom{C}{x})$ \hfill \myanswer{Válido
\begin{fitchproof}
\hypo{Aab}{\forall x (\atom{A}{x} \eif \atom{B}{x})}
...
\end{fitchproof}}
\item $\exists x(\atom{D}{x} \eor \atom{E}{x}), \forall x(\atom{D}{x} \eif \atom{F}{x}) \therefore \exists x(\atom{D}{x} \eand \atom{F}{x})$ \hfill \myanswer{Inválido\\
Contra-interpretação: tome como domínio o número $1$. Faça $Dx$ não valer para nada. Faça $Ex$ e $Fx$ valerem para tudo.}
\item $\forall x\forall y(\atom{R}{x,y} \eor \atom{R}{y,x}) \therefore \atom{R}{j,j}$ \hfill \myanswer{Válido
\begin{fitchproof}
...
\item $\exists x\exists y(\atom{R}{x,y} \eor \atom{R}{y,x}) \therefore \atom{R}{j,j}$ \hfill \myanswer{Inválido\\
Contra-interpretação: considere o diagrama a seguir, fazendo $j$ designar 2.
\begin{center}
\begin{tikzpicture}
\node (atom4) at (0,0) {1};
\node (atom5) at (2,0) {2};
...
número 2 não está conectado por nenhuma seta.}
\end{center}}
\item $\forall x\,\atom{P}{x} \eif \forall x\,\atom{Q}{x}, \exists x \enot \atom{P}{x} \therefore \exists x \enot \atom{Q}{x}$ \hfill \myanswer{Inválido\\
Contra-interpretação: tome como domínio o número $1$. Faça $\atom{P}{x}$ não ser verdadeira para nada. Faça $\atom{Q}{x}$ ser verdadeira para tudo.}

\item $\exists x\,\atom{M}{x} \eif \exists x\,\atom{N}{x}$, $\enot \exists x\,\atom{N}{x}\therefore  \forall x \enot \atom{M}{x}$\hfill \myanswer{Válido\\
\begin{fitchproof}
...
\chapter{Conversão de quantificadores}\setcounter{ProbPart}{0}
\problempart
...
\problempart
Mostre que cada par de sentenças é interderivável: %TB: Novamente, foi preciso fazer alguma numeração manual para que as multicols tivessem espaço suficiente.
...
\problempart
Em \cref{s:MoreMonadic}, examinamos o que acontece quando movemos quantificadores \textquotedblleft{}através\textquotedblright{} de vários operadores lógicos. Mostre que cada par de sentenças é interderivável:
\begin{compactlist}
...
\chapter{Regras para a identidade}\setcounter{ProbPart}{0}
\problempart
\label{pr.identity}
Para cada uma das afirmações a seguir, forneça uma demonstração na LPO que mostre que ela é verdadeira.
...
\problempart
Mostre que as sentenças a seguir são interderiváveis:
\begin{itemize}
\item $\exists x \bigl([\atom{F}{x} \eand \forall y (\atom{F}{y} \eif x = y)] \eand x = n\bigr)$
\item $\atom{F}{n} \eand \forall y (\atom{F}{y} \eif n= y)$
\end{itemize}
E, portanto, que ambas podem plausivelmente simbolizar a sentença em português \textquotedblleft{}Nick é o F\textquotedblright{}.

\noindent\myanswer{Em uma direção:
\begin{fitchproof}
...
\myanswer{Agora na outra direção:
\begin{fitchproof}
...
\problempart
Em \cref{sec.identity}, afirmamos que as seguintes são simbolizações logicamente equivalentes da sentença \textquotedblleft{}existe exatamente um F\textquotedblright{}:
\begin{itemize}
...
\end{itemize}
Mostre que todas são interderiváveis. (\emph{Dica}: para mostrar que três afirmações são interderiváveis, basta mostrar que a primeira prova a segunda, a segunda prova a terceira e a terceira prova a primeira; pense por quê.)\\
\myanswer{Basta mostrar que a primeira prova a segunda, a segunda prova a terceira e a terceira prova a primeira, pois então poderemos mostrar que qualquer uma prova qualquer outra simplesmente encadeando as demonstrações (numerando as linhas quando necessário). Com isso, começamos a primeira demonstração:
\begin{fitchproof}
...
\
\\\myanswer{Agora a segunda demonstração:
\begin{fitchproof}
...
\noindent\myanswer{E, por fim, a terceira demonstração:
\begin{fitchproof}
...
Simbolize o seguinte argumento
	\begin{earg}
		...
		\item[\texttherefore] Existem exatamente duas coisas que são F ou G.
	\end{earg}
E ofereça uma demonstração.\\
\myanswer{
\begin{earg}
\item $\exists x \bigl[\atom{F}{x} \eand \forall y (\atom{F}{y} \eif x = y)\bigr]$
...
\problempart
Ofereça demonstrações que justifiquem a adição da segunda e da quarta regras CQ como regras derivadas.


\noindent\myanswer{Justificativa para a segunda regra:
\begin{fitchproof}
...
\end{fitchproof}
A quarta regra é mais difícil de justificar. Eis uma demonstração relativamente direta, mas que usa a regra derivada DNE:
...
\end{fitchproof}
Eis uma demonstração que não usa regras derivadas:
\begin{fitchproof}
	\hypo{nAa}{\enot \forall x\,\atom{A}{x}}
	\open
