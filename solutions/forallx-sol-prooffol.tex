...
\problempart
Explique por que essas duas «demonstrações» estão incorretas. Além disso, forneça interpretações que invalidem as formas argumentativas falaciosas que essas «demonstrações» consagram:
\begin{multicols}{2}
...
\emph{muito} mais fácil se você simbolizar, por exemplo, «Nenhum F é G» como
...
Aristóteles e seus sucessores identificaram outras formas silogísticas que dependiam da «importação existencial». Simbolize cada uma das formas argumentativas a seguir na LPO e ofereça demonstrações.
\begin{compactlist}
...
\problempart
Em \cref{s:MoreMonadic}, examinamos o que acontece quando movemos quantificadores «através» de vários operadores lógicos. Mostre que cada par de sentenças é interderivável:
\begin{compactlist}
...
\end{itemize}
E, portanto, que ambas podem plausivelmente simbolizar a sentença em português «Nick é o F».
...
\problempart
Em \cref{sec.identity}, afirmamos que as seguintes são simbolizações logicamente equivalentes da sentença «existe exatamente um F»:
\begin{itemize}
